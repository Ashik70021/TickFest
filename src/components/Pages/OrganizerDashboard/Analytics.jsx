...
const Analytics = () => {
    ...
    const calculateAnalytics = () => {
        ...
        events.forEach(event => {
            ...
            const ticketsSold = event.ticketTypes?.reduce((acc, ticket) => {
        ...
        analytics.averageTicketPrice = analytics.totalTicketsSold > 0 
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-white/80 backdrop-blur-lg rounded-2xl p-6 shadow-xl border border-white/20">
                    ...
                    <p className="text-3xl font-black text-gray-800">{analytics.totalEvents}</p>
                ...
                <div className="bg-white/80 backdrop-blur-lg rounded-2xl p-6 shadow-xl border border-white/20">
                    <div className="flex items-center justify-between mb-3">
                        ...
                        <span className="text-xs text-green-600 font-semibold bg-green-100 px-2 py-1 rounded-full">Sold</span>
                    </div>
                    <p className="text-3xl font-black text-gray-800">{analytics.totalTicketsSold.toLocaleString()}</p>
                ...
                <div className="bg-white/80 backdrop-blur-lg rounded-2xl p-6 shadow-xl border border-white/20">
                    <div className="flex items-center justify-between mb-3">
                        ...
                        <span className="text-xs text-purple-600 font-semibold bg-purple-100 px-2 py-1 rounded-full">Revenue</span>
                    </div>
                    <p className="text-3xl font-black text-gray-800">৳{analytics.totalRevenue.toLocaleString()}</p>
                ...
                <div className="bg-white/80 backdrop-blur-lg rounded-2xl p-6 shadow-xl border border-white/20">
                    ...
                    <p className="text-3xl font-black text-gray-800">৳{analytics.averageTicketPrice}</p>
            ...
            {analytics.topEvent && (
                <div className="bg-white/80 backdrop-blur-lg rounded-3xl p-6 shadow-xl border border-white/20">
                    <h3 className="text-xl font-bold text-gray-800 mb-6">🏆 Top Performing Event</h3>
                    
                    <div className="bg-gradient-to-r from-[#B13BFF]/10 to-[#471396]/10 rounded-2xl p-6 border border-[#B13BFF]/20">
                        <div className="flex flex-col lg:flex-row lg:items-center gap-6">
                            <div className="flex-1">
                                <h4 className="text-2xl font-bold text-gray-800 mb-2">{analytics.topEvent.name}</h4>
                                <div className="flex flex-wrap gap-4 text-sm text-gray-600 mb-4">
                                    <span className="flex items-center gap-1">📅 {analytics.topEvent.date} {analytics.topEvent.month} {analytics.topEvent.year}</span>
                                    <span className="flex items-center gap-1">📍 {analytics.topEvent.venue}</span>
                                    <span className="flex items-center gap-1">🎭 {analytics.topEvent.category}</span>
                                </div>
                            </div>
                            
                            <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 lg:w-80">
                                <div className="text-center">
                                    <p className="text-2xl font-black text-green-600">৳{analytics.topEvent.revenue.toLocaleString()}</p>
                                    <p className="text-xs text-gray-600">Revenue</p>
                                </div>
                                <div className="text-center">
                                    <p className="text-2xl font-black text-blue-600">{analytics.topEvent.ticketsSold}</p>
                                    <p className="text-xs text-gray-600">Tickets Sold</p>
                                </div>
                                <div className="text-center">
                                    <p className="text-2xl font-black text-purple-600">
                                        ৳{analytics.topEvent.ticketsSold > 0 ? Math.round(analytics.topEvent.revenue / analytics.topEvent.ticketsSold) : 0}
                                    </p>
                        ...
                        <div key={category} className="bg-gray-50/50 rounded-2xl p-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="font-semibold text-blue-600">{data.ticketsSold}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-sm text-gray-600">Avg per Event</span>
                                    <span className="font-semibold text-purple-600">
                                        ৳{data.events > 0 ? Math.round(data.revenue / data.events).toLocaleString() : 0}
                                    </span>
                                </div>
                            </div>
                        </div>
            ...
            {/* Sales Trend */}
            <div className="bg-white/80 backdrop-blur-lg rounded-3xl p-6 shadow-xl border border-white/20">
                <h3 className="text-xl font-bold text-gray-800 mb-6">Daily Sales Trend (Last 14 Days)</h3>
                
                <div className="space-y-3">
                    {analytics.salesTrend.map((day, index) => {
                        const maxSales = Math.max(...analytics.salesTrend.map(d => d.sales));
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-white/80 backdrop-blur-lg rounded-3xl p-6 shadow-xl border border-white/20">
                    <h3 className="text-xl font-bold text-gray-800 mb-6">Audience Insights</h3>
                    
                    <div className="space-y-6">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="text-center">
                                <p className="text-2xl font-black text-blue-600">{analytics.audienceInsights.totalAttendees}</p>
                                <p className="text-sm text-gray-600">Total Attendees</p>
                            </div>
                            <div className="text-center">
                                <p className="text-2xl font-black text-green-600">{analytics.audienceInsights.repeatCustomers}%</p>
                                <p className="text-sm text-gray-600">Repeat Customers</p>
                            </div>
                        </div>
                        
                        <div className="text-center">
                            <p className="text-2xl font-black text-purple-600">{analytics.audienceInsights.averageAge}</p>
                        ...
                        <div>
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="font-semibold text-blue-600">{analytics.audienceInsights.genderSplit.male}%</span>
                                </div>
                                <div className="bg-gray-200 rounded-full h-2">
                                    <div 
                                        className="bg-blue-500 h-2 rounded-full"
                                        style={{ width: `${analytics.audienceInsights.genderSplit.male}%` }}
                                    ></div>
                                </div>
                                
                                <div className="flex items-center justify-between mt-3">
                                    <span className="text-sm text-gray-600">Female</span>
                                    <span className="font-semibold text-pink-600">{analytics.audienceInsights.genderSplit.female}%</span>
                                </div>
                                <div className="bg-gray-200 rounded-full h-2">
                                    <div 
                                        className="bg-pink-500 h-2 rounded-full"
                                        style={{ width: `${analytics.audienceInsights.genderSplit.female}%` }}
                                    ></div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="bg-yellow-50 p-4 rounded-2xl">
                            ...
                            <p className="text-sm text-yellow-700">
                                Consider adding more VIP tickets - they have 85% sell-through rate
                            </p>
                        </div>
                    </div>
                </div>
            ...
            {analytics.totalEvents === 0 && (
                <div className="text-center py-12">
                    ...
                    <h3 className="text-xl font-bold text-gray-600 mb-2">No analytics data available</h3>
                    <p className="text-gray-500">
                        Add some events to your dashboard to see detailed analytics and insights.
                    </p>
                </div>
            )}
        </div>
