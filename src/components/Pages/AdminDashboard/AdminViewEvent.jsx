...
const AdminViewEvent = () => {
    ...
    useEffect(() => {
        const fetchEventData = async () => {
            try {
                setLoading(true);
                const response = await axios.get(`${API_BASE_URL}/api.php/api/events/${id}`);
                ...
                <div className="mb-8">
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            ...
                            <p className="text-gray-600">Comprehensive view of event information</p>
                        </div>
                        ...
                        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
                            <div className="relative h-64 lg:h-80">
                                ...
                                <div className="absolute bottom-4 left-4 right-4">
                                    <h1 className="text-2xl lg:text-3xl font-bold text-white mb-2">{event.name}</h1>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div className="space-y-4">
                                        <h3 className="text-lg font-semibold text-gray-900 flex items-center">
                                            <svg className="w-5 h-5 mr-2 text-[#B13BFF]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                            </svg>
                                            Date & Time
                                        </h3>
                                        <div className="space-y-2">
                                            <p className="text-gray-700"><span className="font-medium">Date:</span> {event.date} {event.month} {event.year}</p>
                                            <p className="text-gray-700"><span className="font-medium">Start Time:</span> {event.time}</p>
                                            {event.endTime && <p className="text-gray-700"><span className="font-medium">End Time:</span> {event.endTime}</p>}
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        <h3 className="text-lg font-semibold text-gray-900 flex items-center">
                                            <svg className="w-5 h-5 mr-2 text-[#471396]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                                            </svg>
                                            Location
                                        </h3>
                                        <div className="space-y-2">
                                            <p className="text-gray-700"><span className="font-medium">Venue:</span> {event.venue}</p>
                                            <p className="text-gray-700"><span className="font-medium">Location:</span> {event.location}</p>
                                            {event.coordinates && (
                                                <p className="text-gray-700">
                                                    <span className="font-medium">Coordinates:</span> {event.coordinates.latitude}, {event.coordinates.longitude}
                                                </p>
                                            )}
                                        </div>
                                    </div>
                        ...
                        {event.organizer && (
                            <div className="bg-white rounded-2xl border border-gray-100 p-8 shadow-sm">
                                <h2 className="text-2xl font-bold text-[#090040] mb-6 flex items-center">
                                    <svg className="w-6 h-6 mr-3 text-[#B13BFF]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <div className="flex-1">
                                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                            <div className="space-y-3">
                                                <p className="text-gray-700"><span className="font-medium">Name:</span> {event.organizer.name}</p>
                                                <p className="text-gray-700"><span className="font-medium">Email:</span> {event.organizer.email}</p>
                                                {event.organizer.phone && <p className="text-gray-700"><span className="font-medium">Phone:</span> {event.organizer.phone}</p>}
                                            </div>
                                            {event.organizer.bio && (
                                                <div>
                            ...
                            <div className="bg-white rounded-2xl border border-gray-100 p-8 shadow-sm">
                                ...
                                {event.artists.map((artist, index) => (
                                    <div key={index} className={`${index > 0 ? 'mt-6 pt-6 border-t border-gray-200' : ''}`}>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <div>
                                                <p className="text-gray-700"><span className="font-medium">Name:</span> {artist.name}</p>
                                                {artist.genre && <p className="text-gray-700"><span className="font-medium">Genre:</span> {artist.genre}</p>}
                                            </div>
                                            {artist.bio && (
                                                <div>
                                                    <p className="font-medium text-gray-900 mb-2">Bio:</p>
                            ...
                            <div className="bg-white rounded-2xl border border-gray-100 p-8 shadow-sm">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                    {event.gallery.map((image, index) => {
                                        ...
                                        const imagePath = image.image_path || image.path || image.url || image.src || image;
                                        const imageUrl = typeof imagePath === 'string' ? getImageUrl(imagePath) : '/api/placeholder/200/200';
                                        
                                        return (
                                            <div key={index} className="aspect-square overflow-hidden rounded-lg border border-gray-200 group relative">
                                                <img
                                                    src={imageUrl}
                                                    alt={`Event gallery ${index + 1}`}
                                                    className="w-full h-full object-cover hover:scale-105 transition-transform duration-300 cursor-pointer"
                                                    onError={(e) => {
                                                        console.log(`Failed to load gallery image ${index}:`, imagePath);
                        ...
                        <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                            ...
                            <div className="space-y-4">
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="font-semibold text-gray-900">{getTotalTickets(event)}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-gray-600">Tickets Sold</span>
                                    <span className="font-semibold text-green-600">{getTicketsSold(event)}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-gray-600">Revenue</span>
                                    <span className="font-semibold text-[#471396]">৳{getEventRevenue(event).toLocaleString()}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-gray-600">Status</span>
                                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${event.status === 'Active' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                                        {event.status}
                                    </span>
                                </div>
                            </div>
                        ...
                        {event.ticketTypes && event.ticketTypes.length > 0 && (
                            <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                                <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                                    <svg className="w-5 h-5 mr-2 text-[#B13BFF]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a1 1 0 001 1h1a1 1 0 100 2H4a1 1 0 00-1 1v3a2 2 0 002 2h14a2 2 0 002-2v-3a1 1 0 00-1-1h-1a1 1 0 100-2h1a1 1 0 001-1V7a2 2 0 00-2-2H5z" />
                                    </svg>
                                    Ticket Types
                                </h3>
                                <div className="space-y-4">
                                    {event.ticketTypes.map((ticket, index) => (
                                        <div key={index} className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                                            <div className="flex justify-between items-start mb-2">
                                                ...
                                                <span className={`px-2 py-1 rounded-full text-xs font-medium ${ticket.available ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                                            ...
                                            <div className="space-y-1 text-sm">
                                                ...
                                                <div className="flex justify-between">
                                                    ...
                                                    <span className="font-medium">{ticket.remaining || 0}</span>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
